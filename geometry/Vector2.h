...
namespace Core {
  ...
  template <typename T, bool customStorage, typename Enable = void> class Vector2;

  template <typename T, bool customStorage>
  class Vector2<T, customStorage, Core::enable_if_t<Core::is_numeric<T>::value>> : public VectorStorage<T, VECTOR2_COMPONENT_COUNT, customStorage>, public Vector2Components<T> {
